### Kubernetes Control Plane/Master Setup
#!/bin/bash
#
...
if [[ "$PUBLIC_IP_ACCESS" == "false" ]]; then
    ...
    sudo kubeadm init --control-plane-endpoint="$MASTER_PUBLIC_IP" --apiserver-cert-extra-sans="$MASTER_PUBLIC_IP" --pod-network-cidr="$POD_CIDR" --node-name "$NODENAME" --cri-socket unix:///var/run/crio/crio.sock --ignore-preflight-errors Swap
